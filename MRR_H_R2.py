...
base_directory = pathlib.Path(directory) / test_name
...
for option_path in tqdm.tqdm(options_files):
    ...
    if 'window' not in options:
        options['window'] = 0
    options['system'] = options['sys'] + options['algorithm'] + str((options['window'] if 'window' in options else 0))
...
dataset = 'BMSClean'
# systems_to_compare = ['OKOK-1', 'arfarf-1', 'linlinear-1', 'normSCGnormSCG-1', 'temptemporal-1', 'treetree-1', 'sysAirStream-Backtrack:True1500', 'sysAirStream-Backtrack:False1500']
systems_to_compare = ['sysdynse0', 'OKOK-1', 'arfarf-1', 'linlinear-1', 'normSCGnormSCG-1', 'temptemporal-1', 'sysAirStream-Backtrack:True1500', 'sysAirStream-Backtrack:False1500']
systems_to_compare_names = ['DYNSE', 'OK', 'ARF', 'IDW', 'NORM', 'NC', '$AS_r$', '$AS_b$']
df_t = df
# df_t['system'] = df_t['sys'] + df_t['algorithm'] + df_t['window'].astype(str)
df_g = df_t.groupby(['dataset', 'run', 'system']).agg('mean')
...
df_rank_long = df_rank.reset_index()
df_rank_long = df_rank_long.loc[df_rank_long['system'].isin(['sysdynse0', 'OKOK-1', 'arfarf-1', 'linlinear-1', 'normSCGnormSCG-1', 'temptemporal-1', 'treetree-1', 'sysAirStream-Backtrack:True1500', 'sysAirStream-Backtrack:True2500', 'sysAirStream-Backtrack:False1500', 'sysAirStream-Backtrack:False2500'])]
df_rank_long['ranking'] = df_rank_long.groupby('run')[measure].rank(ascending=False)
df_rank_gb= df_rank_long.groupby(['dataset', 'run', 'system']).mean()
df_rank_u = df_rank_gb.unstack('system')
df_rank_u.xs(dataset).loc[:, (slice(None), systems_to_compare)]
#%%
# r = df_rank_u.xs('RangioraClean')[['OKOK-1', 'arfarf-1', 'linlinear-1', 'normSCGnormSCG-1', 'temptemporal-1', 'treetree-1', 'sysAirStream-Backtrack:True1500', 'sysAirStream-Backtrack:True2500', 'sysAirStream-Backtrack:False1500', 'sysAirStream-Backtrack:False2500']]
# r['OKOK-1'] = r['OKOK-1'].fillna(r['OKOK-1'].min())
# systems_to_compare = ['sysAirStream-Backtrack:False1500', 'sysAirStream-Backtrack:True1500']
...
print(cd)
graph_ranks(means, systems_to_compare_names, cd, width = 10, textspace = 3, filename = f"{dataset}-{measure}_R2")
# graph_ranks(means, systems_to_compare_names, cd)

# %%
list(df['system'].unique())

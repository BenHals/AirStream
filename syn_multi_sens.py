...
class ReverseRBFGenerator:
    def __init__(self, difficulty, reverse = False, tree_random_state = None, sample_random_state = None):
        self.stream = RandomRBFGenerator(n_centroids=difficulty*2, n_classes=2, n_features=8, model_random_state=tree_random_state, sample_random_state=sample_random_state)
        self.reverse = reverse
        self.tree_random_state = tree_random_state
        self.sample_random_state = sample_random_state
        self.n_samples = self.stream.n_samples
...
def process(options):
    ...
    get_results(stream, classifier, output_path, length, 'AirStreamBacktrack')
    ...
    # stream = DataStream(df)
    # stream.prepare_for_use()
    # get_results(stream, tree_classifier, output_path, length, 'RF9', 9000)
...
if __name__ == "__main__":
    ...
    if args.single:
        ...
        option_list = []
        for css in [0.05, 0.15, 0.25, 0.35, 0.45, 0.55, 0.65, 0.75, 0.85, 0.95]:
            for seed in range(3):
                for gen_seed in range(9):
                    o = {**options}
                    o['conf_sensitivity_sustain'] = css
                    o['seed'] = seed
                    o['gen_seed'] = gen_seed
                    option_list.append(o)
        for csd in [0.05, 0.15, 0.25, 0.35, 0.45, 0.55, 0.65, 0.75, 0.85, 0.95]:
            for seed in range(3):
                for gen_seed in range(9):
                    o = {**options}
                    o['conf_sensitivity_drift'] = csd
                    o['seed'] = seed
                    o['gen_seed'] = gen_seed
                    option_list.append(o)
        # for atp in [50, 250, 500, 750, 1000, 1250, 1500, 1750, 2000, 2600, 3000]:
        #     for seed in range(3):
        #         for gen_seed in range(9):
        #             o = {**options}
        #             o['alt_test_period'] = atp
        #             o['alt_test_length'] = atp
        #             o['seed'] = seed
        #             o['gen_seed'] = gen_seed
        #             option_list.append(o)
        # for atl in [50, 250, 500, 750, 1000, 1250, 1500, 1750, 2000]:
        #     for seed in range(3):
        #         for gen_seed in range(9):
        #             o = {**options}
        #             o['alt_test_period'] = 2000
        #             o['alt_test_length'] = atl
        #             o['seed'] = seed
        #             o['gen_seed'] = gen_seed
        #             option_list.append(o)
        for w in [50, 250, 500, 750, 1000, 1250, 1500, 1750, 2000]:
            for seed in range(3):
                for gen_seed in range(9):
                    o = {**options}
                    o['window'] = w

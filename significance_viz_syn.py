...
# test_name = r"synthetic_tests_NORF"
directory = r'H:\PhD\Paper2-AdaptionRepair'
# test_name = r"synthetic_tests_ABCD"
test_name = r"synthetic_tests_R2 - Copy"
...
df_res = df_gb[['acc', 'kt', 'ktc', 'ks', 'ks_detect', 'ks_detect_hd2', 'f1', 'time']] * 100
...
df_2 = df_2.reindex(['sys-dynse-0', 'NC', 'OK', 'RF', 'RF2', 'RFPT_AM', 'ARF', 'AirStreamNoBacktrack', 'AirStreamBacktrack'])
...
df_2 = df_2.reindex(['ks_detect', 'f1', 'time'], axis = 1, level = 1)
...
# %%
#From Orange Evaluation library - But that does not handle enough comparisons.
def test(avranks, n, alpha="0.05", test="nemenyi"):
    k = len(avranks)
    ...
    q = d[(test, alpha)]
    print(q)
    cd = q[min(k, len(q)-1)] * (k * (k + 1) / (6.0 * n)) ** 0.5
    return cd
...
# for dataset in ['reverse_RBF']:
    # for measure in ['ks_detect', 'f1', 'time', 'mem']:
    for measure in ['ks_detect', 'f1', 'time']:
        # systems_to_compare = [x for x in df['algorithm'].unique() if x not in ["RF2"]]
        systems_to_compare = ["ARF", "OK", "AirStreamBacktrack", "AirStreamNoBacktrack", "sys-dynse-0", "Gaussian", "IDW", "NC", "RFPT_AM"]
        systems_to_compare_names = ["ARF", "OK", "$AS_r$", "$AS_b$", "DYNSE", "NORM", "IDW", "NC", "RF"]
        ...
        cd = test(means, r.shape[0], test = 'nemenyi')
        print(cd)
        graph_ranks(means, systems_to_compare_names, cd, width = 10, textspace = 3, filename = f"{dataset}-{measure}_v2.pdf")

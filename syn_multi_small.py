...
class ReverseRBFGenerator:
    def __init__(self, difficulty, reverse = False, tree_random_state = None, sample_random_state = None):
        self.stream = RandomRBFGenerator(n_centroids=difficulty*4, n_classes=4, n_features=7, model_random_state=tree_random_state, sample_random_state=sample_random_state)
        self.reverse = reverse
        self.tree_random_state = tree_random_state
        self.sample_random_state = sample_random_state
        self.n_samples = self.stream.n_samples
    ...
    def next_sample(self):
        ...
        # if self.reverse:
        if extra_val > 0.3:
            y = np.array([1]) if self.reverse else np.array([0])
        # else:
        #     if extra_val < 0.5:
        #         y = [not y[0]]
        return X, y
...
def process(options):
    if options['directory'] == 'cwd':
        base_directory = pathlib.Path.cwd()
    else:
        base_directory = pathlib.Path(options['directory'])
    # output_path = pathlib.Path.cwd() / 'synthetic_tests' / options['generator'] / f"{options['gen_seed']}${options['seed']}"
    # output_path = base_directory / 'synthetic_tests_dd_test' / options['generator'] / f"{get_option_filename(options)}"
    output_path = base_directory / 'synthetic_tests_rr_small_4' / options['generator'] / f"{get_option_filename(options)}"
    ...
    if options['generator'] == 'reverse_tree':
        ...
        stream_A_pre = ReverseTreeGenerator(options['difficulty'], False, options['gen_seed'], options["seed"])
        stream_B = ReverseTreeGenerator(options['difficulty'], True, options['gen_seed'], options["seed"])
        stream_B_pre = ReverseTreeGenerator(options['difficulty'], True, options['gen_seed'], options["seed"])
        stream_C = ReverseTreeGenerator(options['difficulty'], False, options['gen_seed'] + 1, options["seed"])
        stream_D = ReverseTreeGenerator(options['difficulty'], True, options['gen_seed'] + 1, options["seed"])
    if options['generator'] == 'reverse_RBF':
        stream_A = ReverseRBFGenerator(options['difficulty'] + 1, False, options['gen_seed'], options["seed"])
        stream_A_pre = ReverseRBFGenerator(options['difficulty'] + 1, False, options['gen_seed'], options["seed"])
        stream_B = ReverseRBFGenerator(options['difficulty'] + 1, True, options['gen_seed'], options["seed"])
        stream_B_pre = ReverseRBFGenerator(options['difficulty'] + 1, True, options['gen_seed'], options["seed"])
        stream_C = ReverseRBFGenerator(options['difficulty'], False, options['gen_seed'] + 1, options["seed"])
        stream_D = ReverseRBFGenerator(options['difficulty'], True, options['gen_seed'] + 1, options["seed"])
    stream_A.prepare_for_use()
    stream_A_pre.prepare_for_use()
    stream_B.prepare_for_use()
    stream_B_pre.prepare_for_use()
    ...
    # stream_ABC = ConceptDriftStream(stream=stream_AB, drift_stream=stream_C, position=options['positions'][1], width=options['widths'][1], random_state=options['seed'])
    stream_ABA = ConceptDriftStream(stream=stream_AB, drift_stream=stream_A, position=options['positions'][1], width=options['widths'][1], random_state=options['seed'])
    # stream_ABCA = ConceptDriftStream(stream=stream_ABC, drift_stream=stream_A, position=options['positions'][2], width=options['widths'][2], random_state=options['seed'])
    # stream_ABCAB = ConceptDriftStream(stream=stream_ABCA, drift_stream=stream_B, position=options['positions'][3], width=options['widths'][3], random_state=options['seed'])
    # stream_ABCABC = ConceptDriftStream(stream=stream_ABCAB, drift_stream=stream_C, position=options['positions'][4], width=options['widths'][4], random_state=options['seed'])
    # stream_ABCABCB = ConceptDriftStream(stream=stream_ABCABC, drift_stream=stream_B, position=options['positions'][5], width=options['widths'][5], random_state=options['seed'])
    # stream_ABACBDA = ConceptDriftStream(stream=stream_ABACBD, drift_stream=stream_A, position=options['positions'][5], width=options['widths'][5], random_state=options['seed'])
    # stream_ABACBD = ConceptDriftStream(stream=stream_ABACB, drift_stream=stream_D, position=options['positions'][4], width=options['widths'][4], random_state=options['seed'])
    # stream_ABACBDA = ConceptDriftStream(stream=stream_ABACBD, drift_stream=stream_A, position=options['positions'][5], width=options['widths'][5], random_state=options['seed'])
    # stream_ABACBDAD = ConceptDriftStream(stream=stream_ABACBDA, drift_stream=stream_D, position=options['positions'][6], width=options['widths'][6], random_state=options['seed'])
    # stream_ABACBDADC = ConceptDriftStream(stream=stream_ABACBDAD, drift_stream=stream_C, position=options['positions'][7], width=options['widths'][7], random_state=options['seed'])
    ...
    stream_ABA.prepare_for_use()
    full_stream = stream_ABA

    save_X, save_y = full_stream.next_sample(options['run_length'])
    ...
    get_results(stream, classifier, output_path, length, 'AirStreamBacktrackAmend')

    stream = DataStream(df)
    stream.prepare_for_use()
    get_results(stream, nobt_classifier, output_path, length, 'AirStreamNoBacktrack')
    ...
    stream = DataStream(df)
    stream.prepare_for_use()
    get_results(stream, linear_classifier, output_path, length, 'IDW')

    norm_classifier = SimpleBaseline('normSCG', x_locs, y_loc, spacial_pattern, bins)
    stream = DataStream(df)
    stream.prepare_for_use()
    get_results(stream, norm_classifier, output_path, length, 'Gaussian')

    nc_classifier = SimpleBaseline('temporal', x_locs, y_loc, spacial_pattern, bins)
    stream = DataStream(df)
    stream.prepare_for_use()
    get_results(stream, nc_classifier, output_path, length, 'NC')

    OK_classifier = SimpleBaseline('OK', x_locs, y_loc, spacial_pattern, bins)
    stream = DataStream(df)
    stream.prepare_for_use()
    get_results(stream, OK_classifier, output_path, length, 'OK')

    tree_classifier = SimpleWindowBaseline('tree', x_locs, y_loc, spacial_pattern)
    stream = DataStream(df)
    stream.prepare_for_use()
    get_results(stream, tree_classifier, output_path, length, 'RF', 4000)
    tree_classifier = SimpleWindowBaseline('tree', x_locs, y_loc, spacial_pattern)
    stream = DataStream(df)
    stream.prepare_for_use()
    get_results(stream, tree_classifier, output_path, length, 'RF2', 2000)
    tree_classifier = SimpleWindowBaseline('tree', x_locs, y_loc, spacial_pattern)
    stream = DataStream(df)
    stream.prepare_for_use()
    for i in range(1000):
        X, y = stream_A_pre.next_sample()
        print(X)
        print(y)
        tree_classifier.partial_fit([X], [y])
    for i in range(1000):
        X, y = stream_B_pre.next_sample()
        tree_classifier.partial_fit([X], [y])
    get_results(stream, tree_classifier, output_path, length, 'RFS', 0)

    arf_classifier = SimpleBaselineSKMF('arf', x_locs, y_loc, spacial_pattern, bins)
    stream = DataStream(df)
    stream.prepare_for_use()
    get_results(stream, arf_classifier, output_path, length, 'ARF')
...
if __name__ == "__main__":
    ...
    options = {
        ...
        'gen_seed': 5,
        'seed': 36,
        # 'positions': [5000, 12000, 19000, 26000, 33000, 40000, 47000, 55000],
        # 'positions': [5000, 10000, 19000, 26000, 33000, 40000],
        'positions': [10000, 20000],
        # 'positions': [5000, 10000],
        # 'widths': [1, 4000, 1, 4000, 1, 4000, 4000, 4000],
        'widths': [1, 4000],
        # 'gts': [0, 1, 0, 2, 1, 3, 0, 3, 2],
        'gts': [0, 1, 0],
        # 'widths': [1, 1],
        # 'run_length': 62000,
        # 'run_length': 47000,
        ...
        "conf_sensitivity_drift": 0.025,
        "conf_sensitivity_sustain": 0.025,
        "alt_test_length": 1000,
        "alt_test_period": 1000,
        "dd": "adwin",
        'commit': get_git_revision_short_hash(),
        "run_baselines": True,
        "directory": args.directory,
        'experiment_varying_parameters': []
    }
    b = get_state_boundaries(options)
    options['boundaries'] = b

    if args.single:
        process(options)
    else:
        option_list = []
        for gen in ['reverse_tree', 'reverse_RBF']:
            for seed in range(2):
                # for gen_seed in range(25, 25 + 25):
                for gen_seed in range(2):
                    # for dd in ['adwin', 'ddm', 'eddm', 'hddm_a', 'hddm_w']:
                    for dd in ['adwin']:
                        o = {**options}
                        o['generator'] = gen
                        o['seed'] = seed
                        o['gen_seed'] = gen_seed
                        o['dd'] = dd
                        o['experiment_varying_parameters'] = ['seed', 'gen_seed', 'dd']
                        option_list.append(o)

        pool = mp.Pool(processes=args.cpu)
        results = pool.map(process, option_list, chunksize=1)
...
# test_name = "synthetic_tests_dd_test"
test_name = "synthetic_tests_dd_test_small_2"
...
df_gb = df.groupby(['generator','dd', 'algorithm']).agg([custom_stdev])
df_gb.columns = df_gb.columns.droplevel(1)
df_gb = df_gb[['ks_detect_hd', 'f1']].stack(0)
df_gb = df_gb.unstack('generator')
df_gb.index.names = ['dd', 'algorithm', 'val']
df_gb = df_gb.unstack('val')
df_gb = df_gb.unstack('algorithm')
# df_gb.to_latex('dd_compare.txt')
#%%
df_gb
#%%

df_gb = df_gb.unstack('generator')
df_gb = df_gb[['ks_detect_hd', 'f1']]
df_gb = df_gb.swaplevel(0, 1, axis=1)
df_gb = df_gb.unstack('algorithm')
# df_gb.columns = pd.MultiIndex.from_tuples([(b, a) for a,b in df_gb.columns])
df_gb
#%%
df_gb.columns
#%%
...
df_2 = df_2.unstack(1)
df_2 = df_2.reindex(['AirStreamNoBacktrack', 'AirStreamBacktrack'])

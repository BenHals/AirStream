...
experiment_directory = pathlib.Path(r"H:\PhD\Paper2-AdaptionRepair\RepairResearchData\clean_experiments_no_sys_dynse\experiments") / data_name
experiment_directory = pathlib.Path(r"S:\PhD\AirStream\experiments") / data_name
...
for k in all_results:
    ...
    if 'backtrack' not in key:
        key['backtrack'] = "-1"
    if 'proactive_sensitivity' not in key:
        key['proactive_sensitivity'] = "-1"
    if 'cl' not in key:
        key['cl'] = -1
    if 'an' not in key:
        key['an'] = -1
    ...
    for fk in pointer_keys:
        if type(pointer[fk]) is dict:
            ...
            del pointer[fk]
#%%
results = pd.DataFrame.from_dict(formatted_results, orient='index')
# results = results[]
print(index_names)
...
#%%
results['feature_set'] = results['feature_set'].astype('str') + results.index.get_level_values('backtrack').astype('str').values
results['feature_set'] = results['feature_set'].astype('str') + results.index.get_level_values('proactive_sensitivity').astype('str').values
...
by_ct = kappa_df.groupby(['dataset_name', 'feature_set']).agg(['mean', 'std'])
by_ct = kappa_df.groupby(['dataset_name', 'dataset_target_index', 'feature_set']).agg(['mean', 'std'])
# print(kappa_df.groupby(['feature_set', 'dataset_name]).size())
print(kappa_df.groupby(['feature_set', 'dataset_name']).mean()[['pa', 'kappa']])
print(kappa_df.groupby(['feature_set', 'dataset_name', 'mask_p']).mean()[['pa', 'kappa']])
print(kappa_df.groupby(['feature_set', 'dataset_name']).size())
